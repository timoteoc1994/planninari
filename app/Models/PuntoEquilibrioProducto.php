    ...
    public function index($proyectoId)
    {
        ...
        $productos = $recetas->map(function ($receta) use ($costosVariables, $config) {
            ...
            if (!$c) {
                ...
                $receta->unidades_semanales = 0; // Calculado después de mensuales
                ...
                // Primero calculamos las unidades mensuales
                $receta->unidades_mensuales = round($receta->unidades_diarias * $config->dias_por_semana * (52/12));
                
                // Ahora calculamos las semanales usando la nueva fórmula: (unidades_mensuales * 12) / 52
                $receta->unidades_semanales = round(($receta->unidades_mensuales * 12) / 52);
        ...
        $productos = $productos->map(function ($receta) use ($unidadesMensualesNecesariasGlobal) {
            ...
            // Recalculamos las semanales necesarias con la nueva fórmula
            $receta->unidades_semanales_necesarias = round(
