  ...
  <AppLayout title="Etapa 3">
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        Etapa 3 - {{ editando ? 'Editar' : 'Registrar' }} Problema y Solución
      </h2>
    </template>
      ...
      <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 bg-white p-8 rounded shadow">
        <form @submit.prevent="submit">
          <div class="mb-4">
            ...
            <textarea v-model="form.problema" class="w-full border rounded px-3 py-2" required></textarea>
          </div>
          <div class="mb-4">
            <label class="block mb-1 font-semibold">Solución</label>
            <textarea v-model="form.solucion" class="w-full border rounded px-3 py-2" required></textarea>
          </div>
          <button
            type="submit"
